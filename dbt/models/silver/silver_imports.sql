...
with import_source as (
	select regexp_replace(lower(unaccent("noMunMinsgUf")), '[^a-z]', '', 'g')	as "city_name_norm"
      ...
      from {{ source('bronze', 'fish_trade_import') }}
),
cities_source as (
	select "id"
...
joined as (
	select i.*
	  ...
	  left
	  join cities_source 	c on i."city_name_norm" = c."city_name_norm"
	  left
	  join countries_source p on i."country_name_norm" = p."country_name_norm"
),
